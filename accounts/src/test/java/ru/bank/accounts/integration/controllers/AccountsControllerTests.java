...
class AccountsControllerTests {
  ...
  void shouldUpdateAccount() throws Exception {
    ...
    when(accountsService.updateAccountByLogin("test", accountDTO)).thenReturn(accountDTO);

    mockMvc.perform(
            put("/v1/accounts/test").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(accountToJSON(accountDTO)))
        .andExpect(status().isOk());
  }
